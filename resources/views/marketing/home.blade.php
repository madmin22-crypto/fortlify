    ...
    <div class="relative overflow-hidden">
        ...
        <div id="free-audit" class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-8">
                ...
                <p class="text-gray-600 dark:text-gray-400 mb-6">Get a free single-page SEO audit. Create a free account to get started - your reports will be saved and accessible anytime.</p>
                ...
                @auth
                    ...
                    @if (!auth()->user()->hasVerifiedEmail())
                        <div class="mb-4 rounded-md bg-yellow-50 dark:bg-yellow-900/50 p-4">
                            <p class="text-sm font-medium text-yellow-800 dark:text-yellow-200">Please verify your email address to run audits. <a href="{{ route('verification.notice') }}" class="underline">Resend verification email</a></p>
                        </div>
                    @endif
                    ...
                    <form action="{{ route('audits.store') }}" method="POST" class="space-y-4">
                        ...
                        <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" {{ !auth()->user()->hasVerifiedEmail() ? 'disabled' : '' }}>
                            Start Free Audit
                        </button>
                    </form>
                @else
                    <div class="text-center space-y-4">
                        <p class="text-gray-600 dark:text-gray-400">Create a free account to start your SEO audit. It only takes a minute.</p>
                        <div class="flex flex-col sm:flex-row gap-4 justify-center">
                            <a href="{{ route('register') }}" class="inline-flex justify-center py-3 px-6 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Create Free Account
                            </a>
                            <a href="{{ route('login') }}" class="inline-flex justify-center py-3 px-6 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Sign In
                            </a>
                        </div>
                    </div>
